...
?>
            <div class="admin-page-head">
                <div>
                    ...
                    <p><?= $isCentralAdmin ? 'Create accounts, update roles, reset passwords, and manage account status.' : 'Review your class and temporarily mute students when needed.'; ?></p>
                </div>
            </div>
            ...
            <section class="admin-panel">
                ...
                <form method="POST" action="<?= htmlspecialchars(url('/admin/users/import'), ENT_QUOTES, 'UTF-8'); ?>" enctype="multipart/form-data">
                    <?= csrf_input(); ?>
            ...
            <section class="admin-panel">
                ...
                <form class="admin-create-form" method="POST" action="<?= htmlspecialchars(url('/admin/users/create'), ENT_QUOTES, 'UTF-8'); ?>">
                    <?= csrf_input(); ?>
                    <div class="field">
                        <label>Name</label>
            ...
            <section class="admin-panel">
                <form class="admin-user-filter" method="GET" action="<?= htmlspecialchars(url('/admin/users'), ENT_QUOTES, 'UTF-8'); ?>">
                    <div class="field">
                        <label>Search</label>
            ...
            <div class="admin-list">
                <?php foreach ($users as $adminUser): ?>
                    ...
                    $canMuteUser = $isCentralAdmin
                        ? ($adminUser['role'] ?? 'student') !== 'admin'
                        : (($adminUser['role'] ?? 'student') === 'student' && ($adminUser['class_name'] ?? '') === ($filters['class_name'] ?? ''));
                    $isAdminOnline = (bool) ((int) ($adminUser['is_online'] ?? 0));
                    $adminStatus = $isAdminOnline ? 'Online' : 'Offline';
                    ?>
                    <form class="admin-card" method="POST" action="<?= htmlspecialchars(url('/admin/users/update'), ENT_QUOTES, 'UTF-8'); ?>">
                        <?= csrf_input(); ?>
                        ...
                        <div class="admin-grid">
                            <div class="field">
                                <label>Access level</label>
                                <select name="role" class="field-select" <?= !$isCentralAdmin ? 'disabled' : ''; ?>>
                                    <?php foreach ($roleOptions as $option): ?>
                            ...
                            <div class="field">
                                <label>Class</label>
                                <select name="class_name" class="field-select" <?= !$isCentralAdmin ? 'disabled' : ''; ?>>
                                    <?php foreach ($classOptions as $option): ?>
                            ...
                            <div class="field">
                                ...
                                <input name="room_name" type="text" value="<?= htmlspecialchars((string) $adminUser['room_name'], ENT_QUOTES, 'UTF-8'); ?>" <?= !$isCentralAdmin ? 'readonly' : ''; ?>>
                            </div>
                            <div class="field">
                                <label>Headline</label>
                                <input name="headline" type="text" maxlength="140" value="<?= htmlspecialchars((string) ($adminUser['headline'] ?? ''), ENT_QUOTES, 'UTF-8'); ?>" <?= !$isCentralAdmin ? 'readonly' : ''; ?>>
                            </div>
                            <div class="field">
                                <label>Temporary password</label>
                                <input name="password" type="password" placeholder="Leave blank to keep current password" <?= !$isCentralAdmin ? 'readonly' : ''; ?>>
                            </div>
                            <div class="field field-span-2">
                                <label>About</label>
                                <textarea name="bio" rows="3" placeholder="Short profile note" <?= !$isCentralAdmin ? 'readonly' : ''; ?>><?= htmlspecialchars((string) ($adminUser['bio'] ?? ''), ENT_QUOTES, 'UTF-8'); ?></textarea>
                            </div>
                        </div>
                    ...
                    <?php if ($canMuteUser): ?>
                        <?php if ($isMuted): ?>
                            <form class="admin-ban-action-form" method="POST" action="<?= htmlspecialchars(url('/admin/users/unmute'), ENT_QUOTES, 'UTF-8'); ?>">
                                <?= csrf_input(); ?>
                                <input type="hidden" name="user_id" value="<?= htmlspecialchars((string) $adminUser['id'], ENT_QUOTES, 'UTF-8'); ?>">
                                <input type="hidden" name="redirect_to" value="<?= htmlspecialchars(url('/admin/users'), ENT_QUOTES, 'UTF-8'); ?>">
                                <button class="admin-toggle-button unban" type="submit"><span></span>Unmute chat</button>
                            </form>
                        <?php else: ?>
                            <form class="admin-ban-action-form" method="POST" action="<?= htmlspecialchars(url('/admin/users/mute'), ENT_QUOTES, 'UTF-8'); ?>">
                                <?= csrf_input(); ?>
                                <input type="hidden" name="user_id" value="<?= htmlspecialchars((string) $adminUser['id'], ENT_QUOTES, 'UTF-8'); ?>">
                                <input type="hidden" name="redirect_to" value="<?= htmlspecialchars(url('/admin/users'), ENT_QUOTES, 'UTF-8'); ?>">
                    ...
                    <?php endif; ?>
                    <?php if ($isCentralAdmin): ?>
                    <form class="admin-ban-action-form" method="POST" action="<?= htmlspecialchars(url('/admin/users/ban-toggle'), ENT_QUOTES, 'UTF-8'); ?>">
                        <?= csrf_input(); ?>
                        <input type="hidden" name="user_id" value="<?= htmlspecialchars((string) $adminUser['id'], ENT_QUOTES, 'UTF-8'); ?>">
            ...
            <?php if (($userPages ?? 1) > 1): ?>
                <nav class="admin-pagination" aria-label="Users pages">
                    <?php for ($p = 1; $p <= (int) $userPages; $p++): ?>
                        <a class="<?= ($userPage ?? 1) === $p ? 'is-active' : ''; ?>" href="<?= htmlspecialchars(url('/admin/users?page=' . $p . '&q=' . urlencode((string) ($filters['q'] ?? '')) . '&class_name=' . urlencode((string) ($filters['class_name'] ?? '')) . '&ban_state=' . urlencode((string) ($filters['ban_state'] ?? ''))), ENT_QUOTES, 'UTF-8'); ?>"><?= $p; ?></a>
                    <?php endfor; ?>
                </nav>
            <?php endif; ?>
